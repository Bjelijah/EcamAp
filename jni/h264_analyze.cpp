...
#define u8 unsigned char
    ...
    static int get_pic_info(unsigned char * buf,int size, int *width, int *height, int *bitrate)
    {
        h264_param_t pH264Param;
        get_pic_width_height(buf, size, &pH264Param);
        *height = pH264Param.pic_height;
        *width = pH264Param.pic_width;
    }
...
extern "C"{
    void get_pic_info_fromSPS(unsigned char * buf,int len, int * width ,int *height , int *framerate){
        CParseH264 parseH264;
        parseH264.get_pic_info(buf, len, width, height, framerate);
    }
}